...
<div x-show="mobileOpen" x-cloak class="fixed inset-0 z-50 lg:hidden" style="display: none;">
    ...
    >
        ...
        <nav class="flex flex-col gap-1 text-base font-medium text-zinc-700 dark:text-zinc-300">
            <a href="{{ route('home') }}" wire:navigate @click="mobileOpen = false" class="rounded-lg px-3 py-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-900">{{ __('Home') }}</a>
            <a href="{{ route('articles') }}" wire:navigate @click="mobileOpen = false" class="rounded-lg px-3 py-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-900">{{ __('All Articles') }}</a>
            @foreach ($categories as $category)
